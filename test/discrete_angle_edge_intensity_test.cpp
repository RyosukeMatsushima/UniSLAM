...
TEST(DiscreteAngleEdgeIntensityTest, TestDiscreteAngleEdgeIntensity) {
    ...
    for (int i = 2; i < window_size - 2; i++) {
        cv::Mat intensity = discrete_angle_edge_intensity.getBlockIntensity(cv::Point(i, i), 3, i * angle_resolution, angle_resolution * 0.5f);
    ...
    // test the discretizeAngle function with large range of angle
    for (int i = 2; i < window_size - 2; i++) {
        cv::Mat intensity = discrete_angle_edge_intensity.getBlockIntensity(cv::Point(i, i), 3, i * angle_resolution, 2 * angle_resolution);
