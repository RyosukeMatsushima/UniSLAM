...
protected:
    ...
    OptimizeTest()
        : frame0(Pose3D(), {
            EdgeNode(Eigen::Vector3f(1.0, 0.0, 1.0), Eigen::Vector2f(1.0, 0.0), 0),
            EdgeNode(Eigen::Vector3f(-1.0, 0.0, 1.0), Eigen::Vector2f(1.0, 0.0), 1),
            EdgeNode(Eigen::Vector3f(0.0, 1.0, 1.0), Eigen::Vector2f(0.0, 1.0), 2),
            EdgeNode(Eigen::Vector3f(0.0, -1.0, 1.0), Eigen::Vector2f(0.0, 1.0), 3)
          }),
          frame1(Pose3D(), {
            EdgeNode(Eigen::Vector3f(0.0, 0.0, 1.0), Eigen::Vector2f(1.0, 0.0), 0),
            EdgeNode(Eigen::Vector3f(-2.0, 0.0, 1.0), Eigen::Vector2f(1.0, 0.0), 1),
            EdgeNode(Eigen::Vector3f(0.0, 1.0, 1.0), Eigen::Vector2f(0.0, 1.0), 2),
            EdgeNode(Eigen::Vector3f(0.0, -1.0, 1.0), Eigen::Vector2f(0.0, 1.0), 3)
          }),
          frame2(Pose3D(), {
            EdgeNode(Eigen::Vector3f(2.0, 0.0, 1.0), Eigen::Vector2f(1.0, 0.0), 0),
            EdgeNode(Eigen::Vector3f(0.0, 0.0, 1.0), Eigen::Vector2f(1.0, 0.0), 1),
            EdgeNode(Eigen::Vector3f(0.0, 1.0, 1.0), Eigen::Vector2f(0.0, 1.0), 2),
            EdgeNode(Eigen::Vector3f(0.0, -1.0, 1.0), Eigen::Vector2f(0.0, 1.0), 3)
          }),
          edge0(Eigen::Vector3f(1.0, 0.0, 1.0), Eigen::Vector3f(1.0, 0.0, 0.0), 1.0),
          edge1(Eigen::Vector3f(-1.0, 0.0, 1.0), Eigen::Vector3f(1.0, 0.0, 0.0), 1.0),
          edge2(Eigen::Vector3f(0.0, 1.0, 1.0), Eigen::Vector3f(0.0, 1.0, 0.0), 1.0),
          edge3(Eigen::Vector3f(0.0, -1.0, 1.0), Eigen::Vector3f(0.0, 1.0, 0.0), 1.0) {}

    void setEdges() {
        edge_space_dynamics.set_edge3d(edge0.start_point, edge0.direction, edge0.length);
        edge_space_dynamics.set_edge3d(edge1.start_point, edge1.direction, edge1.length);
        edge_space_dynamics.set_edge3d(edge2.start_point, edge2.direction, edge2.length);
        edge_space_dynamics.set_edge3d(edge3.start_point, edge3.direction, edge3.length);
    }
    ...
    void checkEdgeData(const EdgeData& edge, float translation_error_threshold = 1e-4, float rotation_error_threshold = 1e-4) {
        EXPECT_NEAR(edge.start_point.x(), edge.correct_start_point.x(), translation_error_threshold);
        EXPECT_NEAR(edge.start_point.y(), edge.correct_start_point.y(), translation_error_threshold);
        EXPECT_NEAR(edge.start_point.z(), edge.correct_start_point.z(), translation_error_threshold);

        EXPECT_NEAR(edge.direction.x(), edge.correct_direction.x(), rotation_error_threshold);
        EXPECT_NEAR(edge.direction.y(), edge.correct_direction.y(), rotation_error_threshold);
        EXPECT_NEAR(edge.direction.z(), edge.correct_direction.z(), rotation_error_threshold);

        EXPECT_NEAR(edge.length, edge.correct_length, translation_error_threshold);
    }
};
...
TEST_F(OptimizeTest, optimizeWithoutNoise) {
    ...
    addNoise(frame0, Eigen::Vector3f(1.0f, -1.0f, -1.0f), Eigen::Vector3f(-1.1f, 0.2f, 0.2f));

    addNoise(edge0, Eigen::Vector3f(0.1f, 0.1f, 0.1f), Eigen::Vector3f(0.1f, 0.1f, 0.1f), 0.1f);
    addNoise(edge1, Eigen::Vector3f(0.1f, 0.1f, 0.1f), Eigen::Vector3f(0.1f, 0.1f, 0.1f), 0.1f);
    addNoise(edge2, Eigen::Vector3f(0.1f, 0.1f, 0.1f), Eigen::Vector3f(0.1f, 0.1f, 0.1f), 0.1f);
    addNoise(edge3, Eigen::Vector3f(0.1f, 0.1f, 0.1f), Eigen::Vector3f(0.1f, 0.1f, 0.1f), 0.1f);
