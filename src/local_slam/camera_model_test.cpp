...
TEST(CameraModelTest, getDirectionAndAngle) {
    ...
    cv::Size image_size(500, 1500);

    cv::Mat camera_matrix = (cv::Mat_<double>(3, 3) << 100, 0, float(image_size.width) / 2,
                                                       0, 100, float(image_size.height) / 2,
                                                       0, 0, 1);
    ...
    {
        std::vector<cv::Point3f> points_3d = {point};
